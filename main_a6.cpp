...
void createDatabaseStructure(const string& schemaName, arrr2d* tables, size_t tuplesLimit) {
	...
	for (size_t i = 0; i < tables->numRows; ++i) {
		...
		pkSequenceFile << tocharints(tableName) + std::string(std::to_string(tuplesLimit).length(), '0');
...
arrr2d* select(const std::string& schemaName, const std::string& columns, const std::string& tableNames, const std::string& whereClause) {
...
std::istringstream tableStream(tableNames);
std::string tableName;
cout<<"2";
while (std::getline(tableStream, tableName, ',')) {
trim(tableName);
...
for (size_t j = 0; j < columnList->size; ++j) {
	cout<<"8";
//	if ((columnList->get(j)->value.find('.') == std::string::npos)){break;}
std::string requestedTable = columnList->get(j)->value.substr(0, columnList->get(j)->value.find('.'));
cout<<"S";
std::string requestedColumn = columnList->get(j)->value.substr(columnList->get(j)->value.find('.') + 1);
cout<<"9";
