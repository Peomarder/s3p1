...
if(current->numRows == 0){current->row = new arrr();
current->numCols = newCols;
current->row->setSize(0);
current->row->size = 0;
for (size_t i = 0; i < newRow->size; ++i) {///////////////////////////////////////
	cout<<"current->next->row->size->"<<current->row->size<<" newRow->size->"<<newRow->size<<"value:"<<newRow->get(i)->value<<endl;
...
void trim(string& s) { //kinda mid? idk looks bad idk why its here
...
bool evaluateWhereClause(const string& line, arrr* headers, const string& expression) { //depression 
